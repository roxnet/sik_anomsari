...
<div class="row">
	...
	<form target="" method="post">
		...
		<div class="form-group">
			<label name="tag" for="textarea">Tag :</label>
			<div class="row">
				<div class="col-xs-8">
				<textarea  id="tag" class="form-control" name="tags" rows="1"><?php echo $data['tag'];?></textarea>
				</div>
			</div>
		</div>
		<hr/>
...
$(document).ready(function(){
	...
	$("#submit_content").click(function(){
		...
		var tag	= $('textarea[name=tags]').val();
		$.ajax({
			type	:"POST",
			url		:"../kontrol/crud_admin/crud_content.php",
			data	:"crud=edit&id_anggota="+id_anggota+"&judul="+judul+"&category="+category+"&content="+content+"&tag="+tag,
			success	: function(data){
