		...
		if($_POST['do']=="tambah"){
				...
				$life		= mysqli_real_escape_string($koneksi,$_POST['deathlife']);
					...
					if($pass1 == $pass2){ 									
						...
						$insert = mysqli_query($koneksi, "INSERT INTO anggota(email, password, nickname, name, gender, bloodtype, birthplace, birthdate, address, id_keluarga, job, level_user, status) 
															VALUES('$email','$password', '$nickname', '$name', '$gender', '$bloodtype', '$birthplace', '$birthdate', '$adress', '$id_keluarga', '$job', '$level', '$status')") or die(mysqli_error()); // query untuk menambahkan data ke dalam database
